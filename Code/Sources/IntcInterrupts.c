...
{
    void (*handlerFn)() = 0;
    uint16_t irq = INTC.IACKR.B.INTVEC;
    
    handlerFn = INTCInterruptsHandlerTable[irq];
...
void INTC_InitVector()
{
	uint8_t i = 0;
	for(i=0;i<INTC_INTERRUPTS_REQUEST_VECTOR_TABLE_SIZE;i++)
	{
	    /* Set the function pointer in the ISR Handler table */
	    INTCInterruptsHandlerTable[i] = &dummy;
	    /* Set the PSR Priority */
	    INTC.PSR[i].B.PRI = 0; 
	}
}
